...
class OverflowDialog(QDialog):
    def __init__(
        ...
        self.setWindowTitle(
            _text(
                language,
                "Q-Press - Avviso Tabella Attributi",
                "Q-Press - Attribute Table Warning",
            )
        )
        ...
        lbl = QLabel(
            _text(
                ...
                f"essere contenuta nel cartiglio selezionato "
                f"({template_name}).\n\n"
        ...
        self.chk_map = QCheckBox(
            _text(
                language,
                "Mantieni la tavola cartografica",
                "Keep the map sheet",
            )
        )
...
def show_attribute_overflow_dialog(
    row_count, available_mm, estimated_mm, template_name, language="it"
):
    """
    Mostra il dialog per l'overflow e restituisce le opzioni scelte.
    """
    dialog = OverflowDialog(
        row_count, available_mm, estimated_mm, template_name, language
    )
    result = dialog.exec()
