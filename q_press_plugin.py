...
from .layout.layout_builder import (
    build_and_export_layout,
    prepare_topographic_profile_request,
)
            ...
            self.profiles = [
                {
                    "path": "",
                    "title": (
                        self.request.get("titles") or ["Profilo topografico"]
                    )[0],
                    "error": str(error),
                }
            ]
...
class QPressPlugin:
    ...
    def initGui(self):
        ...
        icon_path = os.path.join(
            os.path.dirname(__file__), "resources", "qpress_icon.svg"
        )
        self.action = QAction(
            QIcon(icon_path),
    ...
    def run(self):
        ...
        self._push_message(
            self._text(
                "Shift + Drag per selezionare l'area",
                "Shift + Drag to select the map area",
            ),
        )

    def on_draw_completed(self, rect, start_point, end_point):
        """Handle the extent drawn by the user.
        ...
        if self.capture_mode == "profile":
            QTimer.singleShot(
                0,
                lambda: self.process_profile_export(
    ...
    def process_export(self, rect):
        ...
        if not layer:
            QMessageBox.warning(
                ...
                "Q-Press",
                self._text(
                    "Seleziona un layer attivo.", "Select an active layer."
                ),
            )
            return
        ...
        if not settings["output_dir"]:
            QMessageBox.warning(
                ...
                "Q-Press",
                self._text(
                    "Devi selezionare una cartella di destinazione.",
        ...
        if needs_project_raster:
            QMessageBox.warning(
                ...
                "Q-Press",
                self._text(
                    "Per generare il profilo da progetto devi selezionare un "
                    "raster DTM/DEM caricato nel progetto.",
                    "To generate the project profile, select a DTM/DEM raster "
        ...
        if settings.get("topo_profile", False):
            ...
            self._push_message(
                self._text(
                    "Profilo topografico: esegui un secondo Shift + Drag "
                    "lungo la direzione del profilo",
                    "Topographic profile: perform a second Shift + Drag along "
    ...
    def _run_export(self, layer, rect, settings, map_settings=None):
        ...
        if (
            settings.get("topo_profile", False)
            and "topo_profile_prebuilt" not in settings
        ):
    ...
    def _run_profile_task(self, layer, rect, settings, map_settings):
        ...
        try:
            request = prepare_topographic_profile_request(
                layer, rect, map_settings, settings
            )
        except Exception as error:
            fallback_title = settings.get("topo_profile_single_title")
            if not fallback_title:
                fallback_title = self._text(
                    "Profilo topografico", "Topographic profile", language
                )
        ...
        progress = QProgressDialog(
            self._text(
                "Preparazione profilo topografico...",
                "Preparing topographic profile...",
                language,
            ),
        ...
        def on_finished(result, task):
            ...
            if not result or task.isCanceled():
                ...
                self._push_message(
                    self._text(
                        "Generazione profilo annullata.",
                        "Profile generation canceled.",
                        language,
                    ),
                    "Warning",
                )
        ...
        task.progressChanged.connect(
            lambda value: progress.setValue(int(value))
        )
